    ...
    where TDataService : IDataServiceBase<TEntity>
{
    ...
    protected ApiDataServiceBase(IAppLogging<TDataService> appLogging, IApiServiceWrapperBase<TEntity> serviceWrapper)
    {
        ServiceWrapper = serviceWrapper;
        AppLoggingInstance = appLogging;
    }

    public async Task<IEnumerable<TEntity>> GetAllAsync() => await ServiceWrapper.GetAllEntitiesAsync();

    public async Task<TEntity> FindAsync(int id) => await ServiceWrapper.GetEntityAsync(id);
